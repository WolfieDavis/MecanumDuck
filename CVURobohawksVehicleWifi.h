...
private:
  ...
  // const char vehicleControlHTML[] PROGMEM = 
      ...
      // ;
      const char vehicleControlHTML[] = "No";

  // java script that creates a virtual joystick:
  // const char virtualJoyStickJS[] PROGMEM = "NoNo";
      // #include "CVURobohawksVehicleJoyStickJS.h"
      // ;
  const char virtualJoyStickJS[] = "No";
  ...
  float *mLFptr, *mLBptr, *mRFptr, *mRBptr;
  ...
  void handleJoyStickData()
  {
    DEBUG_SERIAL("updating motor values\n");

    *mLFptr = server.arg("mLF").toInt();
    *mLBptr = server.arg("mLB").toInt();
    *mRFptr = server.arg("mRF").toInt();
    *mRBptr = server.arg("mRB").toInt();
...
public:
  ...
  void setUpWiFi(char *wifiName, char *wifiPass, int channel, float mLF, float mLB, float mRF, float mRB)
  {
